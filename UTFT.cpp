...
// pvallone added
void UTFT::setCharMap(const unsigned char* cm) {
    cMap = cm;
}

void UTFT::setFont(const unsigned char* f, const unsigned int (*d)[3], byte c) {
...
void UTFT::prnt(char *st, int x, int y, int deg){
    int stl, i;
    ...
    for (i=0; i<stl; i++){
        ...
        if(deg == 0){
            ...
            rtChar(*st++, x, y, i, deg); // not supported
        }
...
void UTFT::prntChar(char ch, int x, int y){
...
void UTFT::rtChar (byte c, int x, int y, int pos, int deg){
    // not supported yet
}

/* Function to reverse bits of num */
unsigned int UTFT::reverseBits(unsigned int num)
{
